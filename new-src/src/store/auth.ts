...
import { SignInData, SignUpData, ForgotPasswordData, ResetPasswordData, MagicLinkData, EmailVerificationData, PhoneVerificationData } from '@/types/auth';
...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  verifyPhone: async (phone: string, code: string) => {
    try {
      await axios.post('/api/auth/verify-phone', { phone, code });
    } catch (error) {
